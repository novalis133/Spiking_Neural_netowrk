...
early_stopper = EarlyStopper(patience=5, min_delta=0.10)
##############################################
...
if __name__ == "__main__":
    ...
    # test_net.eval()
    # define optimizer
    optimizer = torch.optim.Adamax(Net_Alpha.parameters(), lr=1e-3, betas=(0.9, 0.999), eps=1e-03)
    ...
    for epoch in range(epochs):
        ...
        # alterantive change
        avg_loss = backprop.BPTT(Net_Alpha, train_loader, optimizer=optimizer, criterion=loss_fn,
...
##############################################
        ...
        print(
            f"SNN: {'Alpha'} ,Epoch {epoch}, Train Loss: {avg_loss.item():.2f}, Train Acc: {train_acc * 100:.2f}%, Test Acc: {test_acc * 100:.2f}%")
        ...
        if early_stopper.early_stop(round(avg_loss.item() ,4)):
            break
    #torch.cuda.empty_cache()
    print('Training using is done with Train Acc: {train_acc * 100:.2f}%, Test Acc: {test_acc * 100:.2f}%')
