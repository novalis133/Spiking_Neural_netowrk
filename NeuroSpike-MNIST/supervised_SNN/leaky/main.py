...
if __name__ == "__main__":
    ...
    for i in SGs:
        ...
        Net_Leaky = nn.Sequential(nn.Conv2d(1, 12, 5),
                                  nn.MaxPool2d(2),
                                  snn.Leaky(beta=beta, spike_grad=i, init_hidden=True),
                                  nn.Conv2d(12, 64, 5),
                                  nn.MaxPool2d(2),
                                  snn.Leaky(beta=beta, spike_grad=i, init_hidden=True),
                                  nn.Flatten(),
                                  nn.Linear(64 * 4 * 4, 10),
                                  snn.Leaky(beta=beta, spike_grad=i, init_hidden=True, output=True)
                                  ).to(device)
        ...
        for epoch in range(epochs):
            ...
            # alterantive change
            avg_loss = backprop.BPTT(Net_Leaky, train_loader, optimizer=optimizer, criterion=loss_fn,
                                     num_steps=num_steps, time_var=False, device=device)

            # print(f"Epoch {epoch}, Train Loss: {avg_loss.item():.2f}")
            # train set accuracy
            train_acc = batch_accuracy(test_loader, Net_Leaky, num_steps)
            ...
            print(
                f"SNN: {'Leaky'}, SGD: {str(retrieve_name(i))} ,Epoch {epoch}, Train Loss: {avg_loss.item():.2f}, Train Acc: {train_acc * 100:.2f}%, Test Acc: {test_acc * 100:.2f}%")

            with open(csv_file, "a", newline="") as file:
                writer = csv.writer(file)
                writer.writerow([str(retrieve_name(i)), 'Leaky', epoch,
