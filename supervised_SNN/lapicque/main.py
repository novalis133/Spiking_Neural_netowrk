...
if __name__ == "__main__":
    ...
    for i in SGs:
        ...
        Net_Lapicque = nn.Sequential(nn.Conv2d(1, 12, 5),
                                     nn.MaxPool2d(2),
                                     snn.Lapicque(beta=beta, spike_grad=i, init_hidden=True),
                                     nn.Conv2d(12, 64, 5),
                                     nn.MaxPool2d(2),
                                     snn.Lapicque(beta=beta, spike_grad=i, init_hidden=True),
                                     nn.Flatten(),
                                     nn.Linear(64 * 4 * 4, 10),
                                     snn.Lapicque(beta=beta, spike_grad=i, init_hidden=True, output=True)
                                     ).to(device)
        ...
        for epoch in range(epochs):
            ...
            print(
                f"SNN: {'Lapicque'}, SGD: {str(retrieve_name(i))} ,Epoch {epoch}, Train Loss: {avg_loss.item():.2f}, Train Acc: {train_acc * 100:.2f}%, Test Acc: {test_acc * 100:.2f}%")

            with open(csv_file, "a", newline="") as file:
                writer = csv.writer(file)
                writer.writerow([str(retrieve_name(i)), 'Lapicque', epoch,
