...
if __name__ == "__main__":
    ...
    # export to csv
    combined_csv.to_csv(f"{(neuron_name)}_csv.csv", index=False, encoding='utf-8-sig')
    # Read the data from the CSV file
    df = pd.read_csv(f"{(neuron_name)}_csv.csv")
    ...
    plt.savefig(f"{(neuron_name)}_chart.png", dpi=350, bbox_inches='tight')
    plt.show()
    plt.close()
